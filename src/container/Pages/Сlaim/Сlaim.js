...
export default class Claim extends React.PureComponent {
    ...
    buildNavMunu = () => {
        ...
        quickFilter.forEach((filter, i) => {
            if(i < 7) {
                menu.push( <ItemNav key={filter.id} img={filter.pictureUri} >{filter.name}</ItemNav>)
            } else {
                subMenu.push(<ItemNav key={filter.id} img={filter.pictureUri} >{filter.name}</ItemNav>)
            }
        });
        ...
        return <>
                {menu}
                    ...
                    <SubMenu show={this.state.show} onClick={() => this.setState({show: !this.state.show})} >
                        {subMenu}
                    </SubMenu>
               </>
    };
    ...
    render() {
        return (
            <Claims>
                <NavBar>Financial literacy</NavBar>
                <ListClaims>
                    {this.buildNavMunu()}
                </ListClaims>
            </Claims>
